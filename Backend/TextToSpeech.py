...
# Check if AssistantVoice is set correctly
if not AssistantVoice:
    print("Error: AssistantVoice is not set in the .env file")
    exit(1)
...
def TTS(Text, func=None, r=True):
    try:
        pygame.mixer.init()  # Initialize pygame mixer outside the try block to avoid multiple initializations
        ...
        while pygame.mixer.music.get_busy():
            if func is not None and func() == False:  # Check if the external function returns False
                break
...
def TextToSpeech(Text, func=lambda: None, r=True):
...
if __name__ == "__main__":
    ...
    while True:
        # Now you can call the TTS function without needing to pass a lambda with arguments
        TextToSpeech(input("Enter the text: "), func=lambda: True)  # Pass a simple lambda here